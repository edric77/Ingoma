...
export function ActivityPlayer({ activities, onComplete }: Props) {
  ...
  return (
    <div className="space-y-4">
      ...
      {current.type === 'flashcard' && <FlashcardActivity activity={current} onResult={handleResult} />}
      ...
      {current.type === 'match' && <MatchActivity activity={current} onResult={handleResult} />}
...
function labelType(t: string) {
  const map: Record<string, string> = {
    mcq: 'QCM',
    truefalse: 'Vrai / Faux',
    flashcard: 'Rappel actif',
    fillblank: 'Texte à trous',
    order: 'Ordonnancement',
    match: 'Association colorée',
    ...
    <div className="space-y-4">
      ...
      {revealed && (
        <>
          ...
          <button type="button" onClick={() => onResult(isCorrect)} className="w-full rounded-xl bg-forest text-white font-medium py-3 flex items-center justify-center gap-2">
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        {[true, false].map((v) => {
          ...
          return (
            <button key={String(v)} type="button" disabled={revealed} onClick={() => { setChoice(v); setRevealed(true) }} className={cn('rounded-xl border py-4 font-medium text-sm transition-colors', style)}>
              {v ? 'Vrai' : 'Faux'}
            </button>
          )
        })}
      ...
      {revealed && (
        <>
          ...
          <button type="button" onClick={() => onResult(!!isCorrect)} className="w-full rounded-xl bg-forest text-white font-medium py-3">Continuer</button>
        </>
      )}
    </div>
  )
  ...
  return (
    <div className="space-y-4">
      <p className="text-ink leading-relaxed">
        {parts[0]}
        <input type="text" value={value} disabled={revealed} onChange={(e) => setValue(e.target.value)} className="inline-block mx-1 w-32 border-b-2 border-forest bg-transparent px-1 text-center font-medium outline-none" placeholder="…" />
        {parts[1] || ''}
      </p>
      {!revealed ? (
        <button type="button" disabled={!value.trim()} onClick={() => setRevealed(true)} className="w-full rounded-xl bg-forest text-white font-medium py-3 disabled:opacity-50">Valider</button>
      ) : (
        <>
          ...
          <button type="button" onClick={() => onResult(ok)} className="w-full rounded-xl bg-forest text-white font-medium py-3">Continuer</button>
        </>
      )}
    </div>
  )
...
function Order({
  ...
  const isCorrect = order.length === activity.correctOrder.length && order.every((v, i) => v === activity.correctOrder[i])
  return (
    <div className="space-y-4">
      ...
      <ul className="space-y-2">
        {order.map((itemIdx, pos) => (
          <li key={itemIdx} className={cn('flex items-center gap-2 rounded-xl border bg-paper-light px-3 py-2.5 text-sm', revealed && (itemIdx === activity.correctOrder[pos] ? 'border-forest bg-forest/10' : 'border-red-flag/40'))}>
      ...
      {!revealed ? (
        <button type="button" onClick={() => setRevealed(true)} className="w-full rounded-xl bg-forest text-white font-medium py-3">Valider l'ordre</button>
      ) : (
        <>
          <div className={cn('rounded-xl border p-3 text-sm', isCorrect ? 'border-forest/30 bg-forest/5' : 'border-red-flag/30 bg-red-flag/5')}>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-2">
        {remaining.map((item) => (
          <button key={item} type="button" onClick={() => setSelected(item)} className={cn('rounded-lg border px-3 py-2 text-sm', selected === item ? 'border-forest bg-forest/10' : 'border-line bg-paper-light')}>
            {item}
          </button>
        ))}
      </div>
      <div className="grid gap-2">
        {activity.categories.map((cat) => (
          <button key={cat.id} type="button" onClick={() => !revealed && assign(cat.id)} className="rounded-xl border border-line bg-paper-light p-3 text-left min-h-[64px]">
            <p className="text-xs font-medium text-forest mb-1">{cat.label}</p>
            <div className="flex flex-wrap gap-1">
              {Object.entries(assignments)
                .filter(([, c]) => c === cat.id)
                .map(([item]) => (
                  <span key={item} className={cn('rounded-md px-2 py-0.5 text-xs', revealed ? (cat.correctItems.includes(item) ? 'bg-forest/20 text-forest' : 'bg-red-flag/20 text-red-flag') : 'bg-line text-ink')}>
                    {item}
                  </span>
                ))}
            </div>
      ...
      {allAssigned && !revealed && (
        <button type="button" onClick={() => setRevealed(true)} className="w-full rounded-xl bg-forest text-white font-medium py-3">Valider</button>
      )}
      {revealed && (
        <>
          <div className={cn('rounded-xl border p-3 text-sm', isCorrect ? 'border-forest/30 bg-forest/5' : 'border-red-flag/30 bg-red-flag/5')}>
      ...
      {!choice ? (
        <div className="space-y-2">
          {activity.choices.map((c) => (
            <button key={c.id} type="button" onClick={() => setChoiceId(c.id)} className="w-full text-left rounded-xl border border-line bg-paper-light px-4 py-3 text-sm hover:border-forest/40">
              {c.label}
            </button>
          ))}
        </div>
        ...
        <>
          ...
          <button type="button" onClick={() => onResult(choice.correct)} className="w-full rounded-xl bg-forest text-white font-medium py-3">Continuer</button>
        </>
      )}
    </div>
  )
